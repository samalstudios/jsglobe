...
const DEFAULT_TINT = '#8a1c3b';
const LEGACY_TINT = '#6f7cff';

const seed = () => [{ id: 'personal', name: 'Personal', kind: 'personal', tint: DEFAULT_TINT, created: Date.now() }];

let list = (storage.get(LIST_KEY, null) ?? seed()).map((item) =>
  item.tint === LEGACY_TINT ? { ...item, tint: DEFAULT_TINT } : item,
);
...
export const workspaces = {
  ...
  create({ name, kind = 'personal', tint = DEFAULT_TINT }) {
