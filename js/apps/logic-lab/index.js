...
class LogicLab extends JGApp {
  ...
  renderApp() {
    ...
    this.$('#bar').items = [
      ...
      { separator: true },
      { id: 'new', label: t('logic-lab.new', 'New'), icon: 'file', iconOnly: true, title: t('logic-lab.startAnEmptyBoard', 'Start an empty board'), action: () => this.#blank() },
      { id: 'undo', label: t('logic-lab.undo', 'Undo'), icon: 'undo', iconOnly: true, title: t('logic-lab.undo', 'Undo'), action: () => this.#undo() },
      { separator: true },
      { id: 'zoom-out', label: t('logic-lab.zoomOut', 'Zoom out'), icon: 'minus', iconOnly: true, title: t('logic-lab.zoomOut', 'Zoom out'), action: () => this.#step(1 / 1.25) },
      { id: 'zoom-fit', label: t('logic-lab.fit', 'Fit'), icon: 'maximize', iconOnly: true, title: t('logic-lab.fitTheBoardToThe', 'Fit the board to the view'), action: () => { this.#fit(); this.#draw(); } },
      { id: 'zoom-in', label: t('logic-lab.zoomIn', 'Zoom in'), icon: 'plus', iconOnly: true, title: t('logic-lab.zoomIn', 'Zoom in'), action: () => this.#step(1.25) },
      { separator: true },
      { id: 'copy-part', label: t('logic-lab.copy', 'Copy'), icon: 'copy', iconOnly: true, title: t('logic-lab.copyTheSelectedPart', 'Copy the selected part'), action: () => this.#copy() },
      { id: 'paste-part', label: t('logic-lab.paste', 'Paste'), icon: 'clipboard', iconOnly: true, title: t('logic-lab.pasteACopy', 'Paste a copy'), action: () => this.#paste() },
      { id: 'rotate', label: t('logic-lab.rotate', 'Rotate'), icon: 'rotate', iconOnly: true, title: t('logic-lab.rotate90DegreesR', 'Rotate 90 degrees (R)'), action: () => this.#turn(90) },
      { id: 'flip', label: t('logic-lab.flip', 'Flip'), icon: 'flip', iconOnly: true, title: t('logic-lab.mirrorLeftToRightF', 'Mirror left to right (F)'), action: () => this.#mirror() },
      { separator: true },
      { id: 'delete', label: t('logic-lab.delete', 'Delete'), icon: 'eraser', iconOnly: true, danger: true, title: t('logic-lab.deleteTheSelection', 'Delete the selection'), action: () => this.#remove() },
      { spacer: true },
      { id: 'import', label: t('logic-lab.openFile', 'Open file'), icon: 'upload', iconOnly: true, title: t('logic-lab.openACircuitFromA', 'Open a circuit from a file'), action: () => this.#importFile() },
      { id: 'export', label: t('logic-lab.saveFile', 'Save file'), icon: 'download', iconOnly: true, title: t('logic-lab.saveThisCircuitToA', 'Save this circuit to a file'), action: () => this.#exportFile() },
