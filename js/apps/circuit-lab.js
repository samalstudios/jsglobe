...
const sheet = css`
  ...
  .scope { border-top: 1px solid var(--border); padding: 10px 12px; display: flex; flex-direction: column; gap: 8px; }
  .scope canvas { height: 132px; border-radius: var(--radius-sm); background: color-mix(in srgb, var(--muted) 70%, transparent); }
  .scope-controls { display: grid; grid-template-columns: auto 1fr; gap: 6px 8px; align-items: center; font-size: 11.5px; }
...
const prefix = (value, unit) => {
  ...
  return `${scaled.toFixed(digits).replace(/\.?0+$/, '')} ${symbol}${unit}`;
};
...
class CircuitLab extends JGApp {
  ...
  renderApp() {
    this.paint(html`<div class="app">
      ...
      <div class="body">
        <div class="board">
          ...
          <div class="hint-bar">Drag on the grid to place a part. Click a part to edit it, then press Delete to remove.</div>
        ...
        <aside class="side">
          ...
          <div class="scope">
            ...
            <div class="scope-controls">
              ...
              <span>Window</span>
              <jg-slider id="window" min="80" max="900" step="20" value="${this.#window}"></jg-slider>
              <span>Scale</span>
              <jg-slider id="range" min="0" max="40" step="1" value="${this.#range}"></jg-slider>
            </div>
            <div class="scope-foot">
              <span class="hint mono tiny" id="scope-scale">auto</span>
              <div class="row tight">
    ...
    this.$('#bar').items = [
      { id: 'select', label: 'Select', icon: 'launcher', select: true, action: () => this.#setTool('select') },
      ...Object.entries(KINDS).map(([kind, meta]) => ({
        id: kind,
        label: meta.label,
        icon: meta.icon,
        select: true,
        action: () => this.#setTool(kind),
      })),
      { spacer: true },
    ...
    this.on(this.$('#signal'), 'change', (event) => {
      this.#signal = event.detail?.value ?? event.target.value;
      this.#trace = [];
      this.#scopeLabel();
    });
    this.on(this.$('#window'), 'input', (event) => {
      this.#window = Number(event.target.value);
      if (this.#trace.length > this.#window) this.#trace = this.#trace.slice(-this.#window);
    });
    this.on(this.$('#range'), 'input', (event) => {
      this.#range = Number(event.target.value);
      this.$('#scope-scale').textContent = this.#range ? `±${this.#range} V` : 'auto';
      this.#scope();
    });
    this.on(this.$('#hold'), 'click', () => {
      this.#hold = !this.#hold;
      this.$('#hold').textContent = this.#hold ? 'Resume' : 'Hold';
    });
  ...
  #point(event) {
    ...
    return [
      Math.round((event.clientX - rect.left) / GRID),
      Math.round((event.clientY - rect.top) / GRID),
    ];
  }
  ...
  #hit(point) {
    return this.#parts.find((part) => {
      ...
      const midX = (ax + bx) / 2;
      const midY = (ay + by) / 2;
      return Math.hypot(point[0] - midX, point[1] - midY) < 0.9;
    });
  }
  ...
  #down(event) {
    ...
    if (this.#tool === 'select') {
      const part = this.#hit(point);
      if (part && part.kind === 'switch' && event.detail === 2) {
        part.closed = !part.closed;
        this.#rebuild();
      }
      this.#selected = part?.id ?? null;
      if (!part) this.#probe = point;
      this.#trace = [];
      this.#inspector();
      this.#draw();
      return;
  ...
  #loop() {
    ...
    const tick = () => {
      if (this.#running && this.#parts.length) {
        for (let index = 0; index < 160; index += 1) this.#circuit.step(dt);
        if (!this.#hold) {
          const sample = this.#sample();
          if (sample !== null) {
            this.#trace.push(sample);
            while (this.#trace.length > this.#window) this.#trace.shift();
          }
        }
  ...
  #draw() {
    ...
    if (this.#drag) {
      ...
      context.lineWidth = 2;
      context.beginPath();
      context.moveTo(this.#drag.from[0] * GRID, this.#drag.from[1] * GRID);
      context.lineTo(this.#drag.to[0] * GRID, this.#drag.to[1] * GRID);
      context.stroke();
      context.restore();
    }
  ...
  #scope() {
    ...
    const peak = this.#range || Math.max(this.#signal === 'current' ? 1e-6 : 0.5, ...this.#trace.map((value) => Math.abs(value)));
    context.strokeStyle = paint.ring;
    context.lineWidth = 1.6;
    context.beginPath();
    this.#trace.forEach((value, index) => {
    ...
    context.fillText(`±${prefix(peak, unit)}`, 6, 12);
    context.textAlign = 'right';
    context.fillText(`${this.#trace.length} samples`, width - 6, 12);
  }
