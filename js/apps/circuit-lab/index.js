...
const t = await appWords('circuit-lab', (lang) => import(`./i18n/${lang}.js`));
...
class CircuitLab extends JGApp {
  ...
  renderApp() {
    ...
    this.$('#bar').items = [
      ...
      { separator: true },
      { id: 'new', label: t('circuit-lab.new', 'New'), icon: 'file', iconOnly: true, title: t('circuit-lab.startAnEmptyBoard', 'Start an empty board'), action: () => this.#blank() },
      { id: 'undo', label: t('circuit-lab.undo', 'Undo'), icon: 'undo', iconOnly: true, title: t('circuit-lab.undo', 'Undo'), action: () => this.#undo() },
      { id: 'redo', label: t('circuit-lab.redo', 'Redo'), icon: 'redo', iconOnly: true, title: t('circuit-lab.redo', 'Redo'), action: () => this.#redo() },
      { separator: true },
      { id: 'zoom-out', label: t('circuit-lab.zoomOut', 'Zoom out'), icon: 'minus', iconOnly: true, title: t('circuit-lab.zoomOut', 'Zoom out'), action: () => this.#step(1 / 1.25) },
      { id: 'zoom-fit', label: t('circuit-lab.fit', 'Fit'), icon: 'maximize', iconOnly: true, title: t('circuit-lab.fitTheCircuitToThe', 'Fit the circuit to the view'), action: () => { this.#touched = false; this.#zoomFit(); } },
      { id: 'zoom-in', label: t('circuit-lab.zoomIn', 'Zoom in'), icon: 'plus', iconOnly: true, title: t('circuit-lab.zoomIn', 'Zoom in'), action: () => this.#step(1.25) },
      { separator: true },
      { id: 'copy-part', label: t('circuit-lab.copy', 'Copy'), icon: 'copy', iconOnly: true, title: t('circuit-lab.copyTheSelectedPart', 'Copy the selected part'), action: () => this.#copy() },
      { id: 'paste-part', label: t('circuit-lab.paste', 'Paste'), icon: 'clipboard', iconOnly: true, title: t('circuit-lab.pasteACopy', 'Paste a copy'), action: () => this.#paste() },
      { id: 'rotate', label: t('circuit-lab.rotate', 'Rotate'), icon: 'rotate', iconOnly: true, title: t('circuit-lab.rotate90DegreesR', 'Rotate 90 degrees (R)'), action: () => this.#turn(90) },
      { id: 'flip', label: t('circuit-lab.flip', 'Flip'), icon: 'flip', iconOnly: true, title: t('circuit-lab.mirrorThePartF', 'Mirror the part (F)'), action: () => this.#mirror() },
      { separator: true },
      { id: 'delete', label: t('circuit-lab.delete', 'Delete'), icon: 'eraser', iconOnly: true, danger: true, title: t('circuit-lab.deleteTheSelectedPart', 'Delete the selected part'), action: () => this.#remove() },
      { spacer: true },
      { id: 'import', label: t('circuit-lab.openFile', 'Open file'), icon: 'upload', iconOnly: true, title: t('circuit-lab.openACircuitFromA', 'Open a circuit from a file'), action: () => this.#importFile() },
      { id: 'export', label: t('circuit-lab.saveFile', 'Save file'), icon: 'download', iconOnly: true, title: t('circuit-lab.saveThisCircuitToA', 'Save this circuit to a file'), action: () => this.#exportFile() },
