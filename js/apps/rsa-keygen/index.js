...
const t = await appWords('rsa-keygen', (lang) => import(`./i18n/${lang}.js`));
...
const ALGORITHMS = {
  ...
  'rsa-4096': { label: t('rsa-keygen.rsa4096', 'RSA 4096'), params: { name: 'RSASSA-PKCS1-v1_5', modulusLength: 4096, publicExponent: new Uint8Array([1, 0, 1]), hash: 'SHA-256' }, uses: ['sign', 'verify'] },
  'rsa-oaep': { label: t('rsa-keygen.rsa2048Encryption', 'RSA 2048 (encryption)'), params: { name: 'RSA-OAEP', modulusLength: 2048, publicExponent: new Uint8Array([1, 0, 1]), hash: 'SHA-256' }, uses: ['encrypt', 'decrypt'] },
  'ec-p256': { label: t('rsa-keygen.ecdsaP256', 'ECDSA P-256'), params: { name: 'ECDSA', namedCurve: 'P-256' }, uses: ['sign', 'verify'] },
  'ec-p384': { label: t('rsa-keygen.ecdsaP384', 'ECDSA P-384'), params: { name: 'ECDSA', namedCurve: 'P-384' }, uses: ['sign', 'verify'] },
  'ecdh-p256': { label: t('rsa-keygen.ecdhP256', 'ECDH P-256'), params: { name: 'ECDH', namedCurve: 'P-256' }, uses: ['deriveKey'] },
};
  ...
  renderApp() {
    this.paint(html`<div class="app">
      <div class="row nowrap">
        ...
        <jg-button id="generate">${t('rsa-keygen.generateKeyPair', 'Generate key pair')}</jg-button>
      </div>
      ...
      <jg-card title="${t('rsa-keygen.jsonWebKey', 'JSON Web Key')}">
