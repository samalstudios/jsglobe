...
class JGWindowLayer extends JGElement {
  ...
  #placeNew(win, options) {
    ...
    const width = clamp(Math.round(bounds.width * 0.62), 360, 1040);
    const height = clamp(Math.round(bounds.height * 0.74), 300, 760);
  ...
  #menu(appId, position) {
    ...
    contextMenu({
      ...
      items: [
        ...
        { label: 'Minimize', glyph: '-', action: () => this.minimize(appId) },
        { separator: true },
        {
        ...
        { separator: true },
        { label: 'Close', glyph: '✕', danger: true, action: () => this.close(appId) },
      ],
    });
  }
