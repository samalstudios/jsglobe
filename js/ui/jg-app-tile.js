...
const sheet = css`
  ...
  a:hover .icon { box-shadow: 0 14px 26px -12px rgba(0, 0, 0, 0.6); }
  .icon svg {
    width: calc(var(--tile, 62px) * 0.46);
    height: calc(var(--tile, 62px) * 0.46);
    stroke-width: 1.6;
  }
  a:hover .icon { transform: translateY(-3px) scale(1.04); }
  a:active .icon { transform: scale(0.94); }
  a:focus-visible .icon { box-shadow: var(--shadow-ring); }
  .label {
...
class JGAppTile extends JGElement {
  ...
  render() {
    ...
    this.style.setProperty('--tint', app.tint);
    this.paint(html`
      <a href="${router.href(`/${app.id}`)}" title="${app.tagline}" draggable="false">
        <span class="shell">
          <span class="icon">${icon(app.icon)}</span>
