...
class JGWidgetBoard extends JGElement {
    ...
    this.paint(html`
      <div class="board">
        ${widgets.map((widget) => {
          ...
          if (!meta) return '';
          return html`<article class="widget" data-size="${widget.size}" data-uid="${widget.uid}" style="--tint:${meta.tint}">
            <header class="head">
  ...
  #menu(uid, x, y) {
    ...
    contextMenu({
      ...
      title: meta.name,
      items: [
        { label: 'Open app', glyph: '↗', action: () => router.app(widget.appId) },
        { separator: true },
        ...
        { separator: true },
        { label: 'Remove widget', glyph: '✕', danger: true, action: () => layout.removeWidget(uid) },
      ],
    });
  }
