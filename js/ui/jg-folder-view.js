  ...
  render() {
    ...
    this.bind('jg-app-tile', 'tile:menu', (event) => {
      ...
      contextMenu({
        ...
        title: meta.name,
        items: [
          { label: 'Move out of folder', glyph: '↥', action: () => layout.removeFromFolder(folder.id, appId) },
          { label: layout.dock().includes(appId) ? 'Remove from dock' : 'Add to dock', glyph: '⌸', action: () => layout.toggleDock(appId) },
