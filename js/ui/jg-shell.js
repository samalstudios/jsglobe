...
const SITE_TITLE = 'JS Globe | Tools';
...
class JGShell extends JGElement {
  ...
  #applyRoute(route) {
    ...
    if (route.name === 'directory') {
      const library = document.createElement('jg-library');
    ...
    if (route.name === 'app') {
      ...
      document.title = `${meta.name} | JS Globe`;
    }
  }
  ...
  #renderMissing(appId) {
    ...
    node.innerHTML = html`
      <div class="missing-card">
        ...
        <p><code>/${appId}</code> isn't installed in this build.</p>
  ...
  #syncRoute({ focused }) {
    ...
    if (focused) {
      if (route.name !== 'app' || route.appId !== focused) router.go(`/${focused}`);
      return;
    }
